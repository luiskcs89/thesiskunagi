...
public abstract class GRequirement
    ...
    private Set<String> usabilityMechanismsIds = new HashSet<String>();

    public final java.util.Set<scrum.client.project.UsabilityMechanism> getUsabilityMechanisms() {
        if ( usabilityMechanismsIds.isEmpty()) return Collections.emptySet();
        return getDao().getUsabilityMechanisms(this.usabilityMechanismsIds);
    }

    public final void setUsabilityMechanisms(Collection<scrum.client.project.UsabilityMechanism> values) {
        usabilityMechanismsIds = ilarkesto.gwt.client.Gwt.getIdsAsSet(values);
        propertyChanged("usabilityMechanismsIds", this.usabilityMechanismsIds);
    }

    public final void addUsabilityMechanism(scrum.client.project.UsabilityMechanism usabilityMechanism) {
        String id = usabilityMechanism.getId();
        if (usabilityMechanismsIds.contains(id)) return;
        usabilityMechanismsIds.add(id);
        propertyChanged("usabilityMechanismsIds", this.usabilityMechanismsIds);
    }

    public final void removeUsabilityMechanism(scrum.client.project.UsabilityMechanism usabilityMechanism) {
        String id = usabilityMechanism.getId();
        if (!usabilityMechanismsIds.contains(id)) return;
        usabilityMechanismsIds.remove(id);
        propertyChanged("usabilityMechanismsIds", this.usabilityMechanismsIds);
    }

    public final boolean containsUsabilityMechanism(scrum.client.project.UsabilityMechanism usabilityMechanism) {
        return usabilityMechanismsIds.contains(usabilityMechanism.getId());
    }
    ...
    public final java.util.List<scrum.client.sprint.AcceptanceCriteria> getAcceptanceCriterias() {
        return getDao().getAcceptanceCriteriasByRequirement((Requirement)this);
    }
