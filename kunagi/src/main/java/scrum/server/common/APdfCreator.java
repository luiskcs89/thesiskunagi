...
public abstract class APdfCreator {
	...
	private void acceptanceCriteriasRow(ATable table, String label, Collection<AcceptanceCriteria> acceptanceCriterias) {
		if (acceptanceCriterias == null || acceptanceCriterias.isEmpty()) return;
		ARow row = table.row();
	...
	private void acceptanceCriterias(APdfContainerElement container, Collection<AcceptanceCriteria> acceptanceCriterias) {
		for (AcceptanceCriteria acceptanceCriteria : acceptanceCriterias) {
			container.nl(miniLabelFont);
			ATable table = container.table(3, 20);

			ARow rowHeader = table.row().setDefaultBackgroundColor(new Color(223, 223, 223));
			rowHeader.cell().setFontStyle(referenceFont).text(acceptanceCriteria.getReference());
			rowHeader.cell().setFontStyle(new FontStyle(defaultFont).setBold(true)).text(acceptanceCriteria.getLabel());
	...
	protected void quality(APdfContainerElement pdf, Quality quality) {
		...
		richtextRow(table, "Quality tests", quality.getTestDescription());
