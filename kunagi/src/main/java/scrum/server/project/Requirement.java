...
public class Requirement extends GRequirement implements Numbered, ReferenceSupport, LabelSupport {
	...
	public List<AcceptanceCriteria> getAcceptanceCriteriasAsList() {
		List<AcceptanceCriteria> accs = new ArrayList<AcceptanceCriteria>(getAcceptanceCriteriasInSprint());
		return accs;
	}

	public Set<AcceptanceCriteria> getAcceptanceCriteriasInSprint() {
		return getAcceptanceCriteriasInSprint(getProject().getCurrentSprint());
	}

	public Set<AcceptanceCriteria> getAcceptanceCriteriasInSprint(Sprint sprint) {
		Set<AcceptanceCriteria> accs = getAcceptanceCriterias();
		Iterator<AcceptanceCriteria> iterator = accs.iterator();
		while (iterator.hasNext()) {
			AcceptanceCriteria acc = iterator.next();
			if (!acc.isSprint(sprint)) iterator.remove();
		}
		return accs;
	}
