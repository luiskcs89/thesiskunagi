...
public class ProjectWorkspaceWidgets extends GProjectWorkspaceWidgets implements ProjectDataReceivedHandler {
	...
	public void showEntity(AGwtEntity entity) {
		...
		if (entity instanceof AcceptanceCriteria) {
			showAcceptanceCriteria((AcceptanceCriteria) entity);
	...
	public AWidget getWidgetForEntity(AGwtEntity entity) {
		...
		if (entity instanceof AcceptanceCriteria)
			return getWidgetForEntity(((AcceptanceCriteria) entity).getRequirement());
	...
	private void showSprintHistory(AcceptanceCriteria acceptanceCriteria) {
		select(sprintHistory);
		sprintHistory.select(acceptanceCriteria);
	}
	...
	private void showAcceptanceCriteria(AcceptanceCriteria acceptanceCriteria) {
		AWidget widget = getWidgetForEntity(acceptanceCriteria);

		if (widget == whiteboard) {
			showWhiteboard(acceptanceCriteria);
		} else if (widget == sprintHistory) {
			showSprintHistory(acceptanceCriteria);
		} else {
			showSprintBacklog(acceptanceCriteria);
		}
	}
	...
	private void showWhiteboard(AcceptanceCriteria acceptanceCriteria) {
		select(whiteboard);
		whiteboard.selectAcceptanceCriteria(acceptanceCriteria);
	}
	...
	private void showSprintBacklog(AcceptanceCriteria acceptanceCriteria) {
		select(sprintBacklog);
		sprintBacklog.selectAcceptanceCriteria(acceptanceCriteria);
	}
