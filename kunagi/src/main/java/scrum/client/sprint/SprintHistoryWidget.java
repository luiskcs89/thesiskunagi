...
public class SprintHistoryWidget extends AScrumWidget {
	...
	public boolean select(AcceptanceCriteria acceptanceCriteria) {
		Sprint sprint = getSprint(acceptanceCriteria.getRequirement());
		if (sprint == null) return false;

		SprintHistorySprintWidget sprintHistorySprintWidget = sprintWidgets.get(sprint);
		return sprintHistorySprintWidget.selectAcceptanceCriteria(acceptanceCriteria);
	}
