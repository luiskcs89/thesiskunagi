...
public class WhiteboardWidget extends AScrumWidget implements TaskBlockContainer, UserHighlightSupport {
	...
	public void selectAcceptanceCriteria(AcceptanceCriteria acceptanceCriteria) {
		if (acceptanceCriteria == null) return;
		BlockListWidget<Requirement> list = getRequirementList(acceptanceCriteria.getRequirement());
		if (list == null) return;
		update();
		RequirementInWhiteboardBlock rBlock = (RequirementInWhiteboardBlock) list.getBlock(acceptanceCriteria
				.getRequirement());
		list.extendBlock(rBlock, true);
		rBlock.selectAcceptanceCriteria(acceptanceCriteria);
	}
